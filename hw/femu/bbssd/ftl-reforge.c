...
static void DiskTrace(FemuCtrl* n) {
    ...
    while (fscanf(fp, "%d", &diskid) != EOF) {
        sprintf(buf, "/home/ubuntu/share/alibabatrace/alibaba_block_traces_2020/sizeGB%d/reforge/%d+1/disk%dprefillReforgePWL%dSUPERL%d", n->tracediskGB, n->rain_stripe_size - 1, diskid, n->pwl, n->superl);
        outfp = fopen(buf, "w");
        printf("outfile %s\n", buf);
...
static void SynthTrace(FemuCtrl* n) {
    ...
    for (int i = 0;i < ROUNDS;i += 1) {
        ...
        while (fscanf(fp, "%d", &full) != EOF) {
            sprintf(buf, "/home/ubuntu/share/alibabatrace/alibaba_block_traces_2020/synthetic/r%gh%gfootprint%dsize20GB%dcycle%d+1/ReforgePWL%dSUPERL%dfull%d", r, h, footprint, traceCycle, n->rain_stripe_size - 1, n->pwl, n->superl, full);
            outfp = fopen(buf, "w");
            printf("outfile %s\n", buf);
