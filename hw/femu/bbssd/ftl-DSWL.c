...
static void* trace(void* arg) {
    ...
    while (fscanf(fp, "%d", &diskid) != EOF) {
        sprintf(buf, "/home/ubuntu/share/alibabatrace/alibaba_block_traces_2020/sizeGB%d/output/disk%dprefillDSWL%d+1", n->tracediskGB, diskid, n->rain_stripe_size - 1);
        outfp = fopen(buf, "w");
        printf("outfile %s\n", buf);
    ...
    //     sprintf(buf, "/home/ubuntu/share/alibabatrace/alibaba_block_traces_2020/synthetic/r0.9h0.1footprint100size20GB8cycle%d+1/DSWLfull%d", n->rain_stripe_size - 1, full);
    //     outfp = fopen(buf, "w");
    //     printf("outfile %s\n", buf);
