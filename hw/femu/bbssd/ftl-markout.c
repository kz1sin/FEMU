...
static void DiskTrace(FemuCtrl* n) {
    ...
    while (fscanf(fp, "%d", &diskid) != EOF) {
        sprintf(buf, "%ssizeGB%d/reforge/%d+1/disk%dprefillECCPWL%d", n->tracepath, n->tracediskGB, n->rain_stripe_size - 1, diskid, n->pwl);
        outfp = fopen(buf, "w");
        printf("outfile %s\n", buf);
...
static void SynthTrace(FemuCtrl* n) {
    ...
    for (int i = 0;i < ROUNDS;i += 1) {
        ...
        while (fscanf(fp, "%d", &full) != EOF) {
            sprintf(buf, "%ssynthetic/r%gh%gfootprint%dsize20GB%dcycle%d+1/ECCPWL%dfull%d", n->tracepath, r, h, footprint, traceCycle, n->rain_stripe_size - 1, n->pwl, full);
            outfp = fopen(buf, "w");
            printf("outfile %s\n", buf);
