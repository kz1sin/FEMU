...
static void SynthTrace(FemuCtrl* n) {
    ...
    while (fscanf(fp, "%d", &full) != EOF) {
        sprintf(buf, "/home/ubuntu/share/alibabatrace/alibaba_block_traces_2020/synthetic/r0.9h0.1footprint100size20GB%dcycle%d+1/PPNRAINPWL%dfull%d", traceCycle, n->rain_stripe_size - 1, n->pwl, full);
        outfp = fopen(buf, "w");
        printf("outfile %s\n", buf);
